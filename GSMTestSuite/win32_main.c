/*
    win32_main.c, adapted from Sam Lantinga's sdl_win32_main.C
...
int console_main(int argc, char *argv[])
{
	...
	bufp = stack_alloc(char, n+1);
	if ( bufp == NULL ) {
		return OutOfMemory();
	}
	strncpy(bufp, appname, n+1); appname[n] = '\0';
	appname = bufp;
...
#endif
{
...
#ifndef NO_STDIO_REDIRECT
	pathlen = GetModuleFileName(NULL, path, arraysize(path));
	...
	bufp = GetCommandLine();
	nLen = strlen(bufp)+1;
	cmdline = stack_alloc(char, nLen);
	if ( cmdline == NULL ) {
		return OutOfMemory();
	}
	strncpy(cmdline, bufp, nLen); bufp[nLen-1] = '\0';
#endif
